...
mtcars |>
    mutate(predicted_mpg = 37.2851 - 5.3445 * wt) |>
    summarize(predictied_mean_mpg = mean(predicted_mpg), observed_mean_mpg = mean(mpg))
```

**There is a very small difference between the mean mpg of the predicted's mpg and the mean mpg of the observed mpg**
...
```{r}
# Write and submit code here!
rand_car <- data.frame(wt = 3.570)
predict(fit_lin, newdata = rand_car)

mtcars |>
    filter(wt == 3.570)
```

**There were 2 cars with the same weight and their mpg did not match the predicted mpg value of 18.2 mpg and this is most likely because of multiple factors that affects the mpg**
...
mtcars |> 
  ...
  geom_point(aes(y = predicted), size = 4, color = "steelblue") +
  labs(x = "Weight (thousands of lbs)",
       y = "Miles per gallon",
...
# Write and submit code here!
mtcars |>
    mutate(residual = resid(fit_lin)) |>
    summarize(mean_residual = mean(residual))
```

**Yes it makes sense to go a value very close to zero or equal to zero since the linear regression model is supposed to be a best-fit line of all the data points so the average residual should be around 0.**
...
The lower the RMSE, the better a model fits a dataset and the more reliable our predicted values can be. (THIS IS VERY IMPORTANT)
...
The higher the $R^2$, the better a model fits a dataset. (THIS IS VERY IMPORTANT)
...
```{r}
# Write and submit code here!
fit_lin_mpg_hp <- lm(mpg ~ hp, data = mtcars)
summary(fit_lin_mpg_hp)

sqrt(mean(resid(fit_lin_mpg_hp)^2))
summary(fit_lin)$adj.r.squared
```

**Based on the Root Mean Square Error (RSME) and Adjusted R-squared, this model (which is based on horsepower `hp`) did a worse job at predicting the mpg**
...
mtcars |>
    mutate(predicted = predict(fit_lin)) |>
    select(mpg, am, predicted)
```

**It makes sense what we get because the `am` variable in the can only be 0 or 1 indicating the status of the transmission of the car and the 2 predicted mpg value are just the mean mpg of the automatic or manual cars**
...
Now the expression of the model is $\widehat{mpg}=37.32155-5.35281*mpg-0.02362∗am$

#### **Try it! Predict the value of `mpg` for an automatic Alfa Romeo Alfetta GTV 2.0 which weighs about 2500 lbs.**

```{r}
# Write and submit code here!
alfa_romeo_alfetta_gtv_2.0 <- data.frame(wt = 2.5, am = 0)

predict(fit_lin, newdata = alfa_romeo_alfetta_gtv_2.0)
```

**The predicted mpg of the automatic Alfa Romeo Alfetta GTV 2.0 which weighs about 2500 lbs is 23.9 mpg **
...
-   Adding too many variables can create issues such as **overfitting**: the model is too specific to the cars in the dataset on which we "train" the model and it will be very difficult to generalize to other cars. (THIS IS VERY IMPORTANT)

-   We can quickly check which features might be more useful for making predicted by looking at the last column in the model output. Any `.` or `*` shows which features are "significant".
