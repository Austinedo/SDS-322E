---
title: "Worksheet 17: Logistic Regression"
output: 
  html_document:
    toc: true
    toc_float: true
...
#### **Try it! Overwrite the `outcome` variable in `biopsy` so that it has value 0 or 1.**
...
**It choose a logistic curve since our outcome variable (y) is coded as a binary variable where 0 represents a benign outcome and 1 represent a malignant outcome.**
...
**We would need to define some cutoff value: for example, if the probability is greater than 0.5 then the tumor should be predicted as malignant, and benign otherwise.**
...
#### **Try it! Create a new variable called `predicted` that takes a value of 1 if the tumor is more likely to be malignant and 0 if it is more likely to be benign. Visualize the logistic regression model as above and color the original data points by the `predicted` value for the outcome. Which points on the graph show True Positive cases? False Positive cases?**
...
biopsy_pred |>
    ggplot(aes(x = clump_thickness, y = outcome)) + 
        geom_smooth(method = 'glm',
                    ...
                    color = 'steelblue',
                    linewidth = 2) +
        geom_point(aes(color = as.factor(predicted)), size = 4, alpha = 0.5) +
        labs(x = 'Clump thickness (scale 1-10)',
             y = 'Outcome (1 = malignant, 0 = benign)',
             title = 'Logistic Regression model to predict malignancy based on clump thickness')
```
...
To quantify performance for logistic regression models, we can consider the ROC curve and the corresponding area under the curve (AUC):
...
What if we add more predictors to our model?
...
-   Adding too many variables can create issues such as **overfitting**: the model can become too specific to the tumors in the dataset on which we "train" the model and it will be very difficult to generalize to other tumors.

-   We can quickly check which features might be more useful for predicting the outcome by looking at the last column in the model output. Any `.` or `*` shows which features are "significant".

#### **Try it! Fit a model with all predictors that make sense. Which predictors seem to be most useful to predict the malignancy of a tumor?**

```{r}
# Write and submit code here!
fit_log_all_vars <- glm(outcome ~ .,
               data = biopsy,
               family = 'binomial')
summary(fit_log_all_vars)
```

**It seems that clump_thickness, marg_adhesion, bare_nuclei, and bland_chromatin are the most significant/useful variables to help predict the malignancy of a tumor.**
